...
?>
	...
	<?php } elseif ($fields['type'] == 'select_pembimbing') {	?>
		...
		<!-- <select class="<?= $fields['key']; ?> form-control form-control-lg" name="<?= $fields['key']; ?>"></select> -->
	...
	<?php } elseif ($fields['type'] == 'file') { ?>
		...
		?>
		<figure style=" center center no-repeat" class="d-flex align-items-center justify-content-center upload-dokumen <?= (form_error('dokumen[' . $id . ']')) ? 'is-invalid' : ''; ?> <?= (($fields['verifikasi'] == 0) && ($pengajuan_status == 4)) ? 'is-invalid' : ''; ?>">
			<?php
			if ($pengajuan_status == 1 && $fields['verifikasi'] == 0 || $pengajuan_status == 4 && $fields['verifikasi'] == 0) {
				if ($thumb) { ?>
					<button id="file-opener-<?= $id; ?>" class="opener hapus btn btn-danger btn-md" type="button"><i class="fas fa-trash"></i> Hapus</button>
				<?php } else { ?>
					<button id="file-opener-<?= $id; ?>" class="opener btn btn-info btn-md" type="button" data-toggle="modal" data-target="#fileUploader"><i class="fas fa-plus"></i> Upload</button>
			<?php }  // $thumb
			} // $pengajuan_status = 1
			?>
